...
int gen_new_E_srcA()
{
    return ((((if_id_curr->icode) == (I_ENTER)) & ((if_id_curr->ifun) == 0)) ? (REG_EBP) : ((if_id_curr->icode) == (I_RRMOVL)||(if_id_curr->icode) == (I_RMMOVL)||(if_id_curr->icode) == (I_ALU)||(if_id_curr->icode) == (I_PUSHL)) ? (if_id_curr->ra) : ((if_id_curr->icode) == (I_POPL)||(if_id_curr->icode) == (I_RET)||(if_id_curr->icode) == (I_ENTER)) ? (REG_ESP) : 1 ? (REG_NONE) : 0);
}
